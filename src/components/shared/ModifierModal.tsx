...
export default function ModifierModal({ item, modifiersObj, onClose, onConfirm }: Props) {
  ...
  // ✨ [리팩토링] 'milkshake' 문자열 대신 requires_flavor_and_size 플래그 사용
  const toggleOption = (option: ModifierOption, groupName: string) => {
    const lowerGroupName = groupName.toLowerCase();
    
    // ✨ [리팩토링] isMilkshake 변수 제거하고 새로운 속성 사용
    const isSingleSelectGroup = item.requires_flavor_and_size && (lowerGroupName.includes('size') || lowerGroupName.includes('flavor'));

    if (isSingleSelectGroup) {
      setSelectedOptions(prev => {
        const group = modifiersObj[groupName];
        // 그룹의 옵션 목록에서 현재 그룹에 속하지 않는 옵션들만 남기고, 새 옵션을 추가합니다.
        const currentGroupOptionNames = group?.options.map(o => o.name) || [];
  ...
  const handleAddToCart = () => {
    // ✨ [리팩토링] item.requires_flavor_and_size 플래그로 필수 옵션 검사
    if (item.requires_flavor_and_size) {
      let hasSize = false;
      let hasFlavor = false;

      // 여전히 그룹 이름에 의존하지만, 아이템 이름 의존성은 제거되었습니다.
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8 bg-gray-900">
          ...
          {safeModifierGroups.map((groupName, idx) => {
            ...
            const sortedOptions = [...group.options].sort((a: any, b: any) => (Number(a.sort_order) || 0) - (Number(b.sort_order) || 0));
